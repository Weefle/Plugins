...
public class ReplyCommand extends Command {
    ...
    public void command(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        if(args.length > 0) {
            String targetName = PlayerData.findByName(sender.getName()).replyTo;
            if(targetName != null) {
                ...
                if(target != null) {
                    ...
                    PlayerData.findByName(target.getName()).replyTo = sender.getName();
                    PlayerData.findByName(sender.getName()).replyTo = target.getName();
                }
                else {
